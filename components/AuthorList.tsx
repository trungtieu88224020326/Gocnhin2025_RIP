    ...
    <div className="relative w-full h-full flex justify-center items-center p-4 lg:p-10 perspective-2000 bg-[#f7f7f7]">
      ...
      <div className="hidden md:flex relative w-full max-w-[1300px] h-[85vh] lg:h-[88vh] max-h-[850px] bg-white shadow-book rounded-sm flex flex-row transform-style-3d border border-border-gray overflow-hidden animate-fade-in">
        ...
        <div className="w-[40%] shrink-0 h-full bg-[#fafafa] relative border-r border-border-gray/60 flex flex-col items-center justify-center p-12 lg:p-20 text-center z-10">
           ...
           <div className="w-12 h-1 bg-[#b42652] mb-12 rounded-full"></div>
           
           {/* Biểu tượng "2025" đã được loại bỏ theo yêu cầu */}
           
           <h2 className="text-2xl lg:text-3xl font-serif font-bold text-[#b42652] mb-4 tracking-tight">Góc nhìn 2025</h2>
           ...
           <div className="w-full pt-10 border-t border-gray-100 flex justify-around">
              <div className="flex flex-col items-center">
                 ...
                 <span className="text-3xl font-serif font-bold text-[#b42652]">{data.length}</span>
              </div>
              ...
              <div className="flex flex-col items-center">
                 ...
                 <span className="text-3xl font-serif font-bold text-[#b42652]">{totalArticles}</span>
              </div>
           </div>
        </div>

        {/* Right Page: Searchable Listing */}
        <div className="flex-1 h-full bg-white relative z-20 flex flex-col overflow-hidden">
           ...
           <div className="px-12 lg:px-20 pt-16 pb-8 shrink-0">
              <div className="flex items-center space-x-3 mb-6">
                 <div className="w-6 h-[1px] bg-[#b42652]"></div>
                 <h2 className="text-[10px] font-sans font-black uppercase tracking-[0.4em] text-[#b42652]">Tìm kiếm</h2>
              </div>
              <div className="relative group">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-[#b42652] transition-colors" size={20} />
                <input type="text" placeholder="Nhập tên tác giả" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-12 pr-12 py-4 bg-[#fcfcfc] border border-gray-100 rounded-xl text-lg font-serif focus:outline-none focus:ring-4 focus:ring-[#b42652]/5 focus:border-[#b42652] transition-all placeholder:text-gray-300" />
           ...
           <div ref={listRef} className="flex-1 overflow-y-auto custom-scroll px-12 lg:px-20 pb-20">
              {filteredAuthors.length > 0 ? (
                        ...
                        <div key={group.author} onClick={() => onSelectAuthor(group)} className="group flex items-center py-6 cursor-pointer hover:bg-gray-50/50 -mx-6 px-6 rounded-xl transition-all duration-300">
                           <div className="shrink-0 mr-6">
                              <div className="w-14 h-14 rounded-full overflow-hidden border border-gray-100 shadow-sm bg-white relative">
                                 {avatar ? (
                                   <img src={avatar} alt={group.author} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" />
                                 ) : (
                           ...
                           <div className="flex-1 min-w-0 pr-6">
                              <h3 className="text-xl font-serif font-bold text-ink group-hover:text-[#b42652] transition-colors truncate">{group.author}</h3>
                           ...
                           <div className="hidden lg:flex items-center space-x-10 mr-10">
                              <div className="text-right">
                                 ...
                                 <span className="text-base font-serif font-bold text-ink group-hover:text-[#b42652] transition-colors">{group.articles.length}</span>
                              </div>
                           </div>
                           <div className="shrink-0 text-gray-200 group-hover:text-[#b42652] transition-all transform group-hover:translate-x-1"><ChevronRight size={24} strokeWidth={1.5} /></div>
                        </div>
                      );
                   })}
                </div>
                ...
                <div className="py-20 flex flex-col items-center justify-center text-center">
                   ...
                   <p className="text-sm text-sub-text mt-1">Vui lòng kiểm tra lại tên tác giả hoặc chức danh.</p>
                </div>
              )}
           </div>
        </div>
