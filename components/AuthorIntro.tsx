...
const AuthorIntro: React.FC<Props> = ({ group, onSelectArticle, onBackToAuthors, authorStats }) => {
  ...
  const shareTitle = `Tác giả: ${group.author} - VnExpress Góc nhìn`;
         ...
         <div className="px-8 pb-8 flex flex-col">
            <div className="flex items-center space-x-4 mb-4">
              ...
              <div className="flex flex-col justify-center min-w-0">
                <h1 className="text-xl font-serif font-bold text-ink leading-tight truncate">{group.author}</h1>
                {authorTitle && <p className="text-sub-text text-[12px] font-sans mt-0.5 truncate opacity-70">{authorTitle}</p>}
              </div>
            </div>
            {authorBio && (
              <div className="mb-6 px-1">
                <div className="text-[13px] text-stone-600 font-serif leading-relaxed italic border-l-2 border-vnexpress/20 pl-4 bio-content" dangerouslySetInnerHTML={{ __html: processedBio }} />
              </div>
            )}
            ...
            <div className="flex justify-between items-center py-4 px-6 bg-vnexpress-light rounded-2xl border border-vnexpress/10">
               <div className="text-center">
                  <span className="block text-[8px] uppercase font-bold text-vnexpress/40 tracking-wider mb-0.5">Bài viết</span>
                  <span className="text-lg font-serif font-bold text-vnexpress">{group.articles.length}</span>
               </div>
               <div className="w-px h-8 bg-vnexpress/10"></div>
               <div className="text-center">
                  <span className="block text-[8px] uppercase font-bold text-vnexpress/40 tracking-wider mb-0.5">Thảo luận</span>
                  <span className="text-lg font-serif font-bold text-vnexpress">{authorStats.comments.toLocaleString()}</span>
               </div>
            </div>
         </div>
         <div className="px-6 py-8 bg-[#f9f9f9] rounded-t-[32px] shadow-[0_-8px_30px_rgba(0,0,0,0.02)] border-t border-gray-100 flex-1">
            <h3 className="font-serif font-bold text-lg text-ink mb-6 px-2">Các tác phẩm tiêu biểu</h3>
            <div className="space-y-3 pb-10">
               {group.articles.map((article) => (
                 <div key={article.id} onClick={() => onSelectArticle(article)} className="flex items-center space-x-4 p-4 rounded-xl bg-white border border-gray-50 hover:border-vnexpress/20 transition-all group active:scale-[0.98] cursor-pointer">
               ...
               <div className="animate-fade-in flex flex-col w-full flex-1 overflow-y-auto custom-scroll pr-2">
                  <div className="flex items-start space-x-6 mb-6">
                    ...
                    <div className="flex flex-col pt-2 min-w-0">
                      <h1 className="text-2xl lg:text-3xl font-serif font-bold text-ink mb-1 tracking-tight leading-tight">{group.author}</h1>
                      {authorTitle && <p className="text-sm lg:text-base text-sub-text font-sans opacity-60">{authorTitle}</p>}
                    </div>
                  </div>
                  {authorBio && (
                    <div className="mb-10">
                      <div className="text-sm lg:text-base text-stone-600 font-serif italic leading-relaxed border-l-2 border-vnexpress/20 pl-6 py-1 bio-content" dangerouslySetInnerHTML={{ __html: processedBio }} />
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-8 w-full py-8 border-y border-gray-50 mb-10">
                     <div className="flex flex-col">
                        ...
                        <span className="text-3xl font-serif font-bold text-vnexpress">{group.articles.length}</span>
                     </div>
                     <div className="flex flex-col">
                        <span className="text-[10px] uppercase font-sans font-black text-gray-300 tracking-[0.2em] mb-1">Thảo luận</span>
                        <span className="text-3xl font-serif font-bold text-vnexpress">{authorStats.comments.toLocaleString()}</span>
                     </div>
                  </div>
               </div>
            </div>
            <div className="flex-1 h-full bg-[#fcfcfc] relative z-20 flex flex-col overflow-hidden">
               ...
               <div className="flex-1 overflow-y-auto custom-scroll px-16 lg:px-20 py-20">
                  <div className="mb-14">
                     ...
                     <h3 className="text-3xl lg:text-[2.5rem] font-serif font-bold text-ink leading-tight tracking-tight">Bài viết tiêu biểu</h3>
                  </div>
                  <div className="space-y-0">
                     {group.articles.map((article, idx) => (
                        <div key={article.id} onClick={() => onSelectArticle(article)} className="group cursor-pointer flex items-center justify-between py-3.5 border-b border-gray-100 hover:bg-white hover:px-6 -mx-6 rounded-xl transition-all duration-300">
